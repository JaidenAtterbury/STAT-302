...
# Use plotly to make the map interactive, also use some html to create the title
# and subtitle:
final_map <- ggplotly(state_map, tooltip = c("text")) %>%
  layout(title = list(text = paste0("Number of Clients Per State",
                                    "<br>",
                                    "<sup>",
                                    "For States in the Dataset", "</sup>")))
# Display the map:
final_map
```
...
```{r echo = FALSE}
chisq.test(regression_set$Category, regression_set$MaritalStatus)
```
